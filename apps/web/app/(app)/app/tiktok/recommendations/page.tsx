...
export const metadata: Metadata = { title: 'TikTok — Recommendations' };
...
export default async function TikTokRecommendationsPage() {
  ...
  const [recs, run] = await Promise.all([
    tiktok.listRecommendations(org.id),
    ...
    <div className="space-y-5">
      ...
      {recs.length === 0 ? (
        <EmptyState
          ...
          description={
            analystConfigured()
              ? 'Run the analysis. Recommendations come only from your synced data, each with evidence, reasoning, confidence, and expected impact — never a guarantee.'
